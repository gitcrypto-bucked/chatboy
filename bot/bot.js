...
const baseUrl = "handler/ai.php";
const sessionId = 104;
const loader = `<span class='loader'><span class='loader__dot'></span><span class='loader__dot'></span><span class='loader__dot'></span></span>`;
const errorMessage = "Minhas desculpas, não estou disponível no momento. =^.^=";
...
const send = (text = "") => {
    fetch(`${baseUrl}?query=${text}&lang=pt&sessionId=${sessionId}&domain=${domain}`, {
            method: "GET",
            dataType: "json",
            headers: {
        ...
        .then(res => {
            if (res.href ==null) {
                ...
                setTimeout(function() {
                    textToSpeech(res.result, loadingDelay + aiReplyDelay);

                }, loadingDelay + aiReplyDelay);
            }

            if (res.href != null) {
                // setResponse('vou te redirecionar ', aiReplyDelay);
                textToSpeech('vou te redirecionar ', aiReplyDelay);
                aiMessageLink(res.href, res.href);
                setTimeout(function() {
...
const aiMessageLink = (content, isLoading = false, delay = 0, href) => {
    setTimeout(() => {
        removeLoader();
        $chatbotMessages.innerHTML += `<li 
      class='is-ai animation' 
